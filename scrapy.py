...
def wiki_scrape(topic_name, verbose=True):
    ...
    wiki_api = wikipediaapi.Wikipedia(language='en', extract_format=wikipediaapi.ExtractFormat.WIKI, user_agent='MyBot/1.0')
...
actors = pd.read_csv('./Best Actors - Top 250.csv', engine='python', encoding='windows-1252')
for actor in actors['Name']:
    print(actor)
    actor_data=wiki_scrape(str(actor),True)
